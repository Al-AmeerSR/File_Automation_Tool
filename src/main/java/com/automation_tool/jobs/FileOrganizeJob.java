...
@Component
public class FileOrganizeJob implements Job {

    private FileService fileService;
    private BatchJobExecutorService batchJobExecutor;

    @Autowired
    public FileOrganizeJob(FileService fileService, BatchJobExecutorService batchJobExecutor) {
        this.fileService = fileService;
        this.batchJobExecutor = batchJobExecutor;
    }

    public FileOrganizeJob() {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (JobUtils.shouldUseSpringBatch(rootDir)) {
            try {
                batchJobExecutor.runOrganizeJob(dataMap);
            } catch (Exception e) {
                throw new JobExecutionException("Spring Batch job execution failed for file organize", e);
            }
        } else {
            fileService.fileOrganize(dataMap);
        }
    }
}
